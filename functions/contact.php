...
// ---------- CONFIG ----------
$RESEND_API_KEY    = getenv('RESEND_API_KEY') ?: 're_iaUkVPMN_DFrsxnMFasTvANPnDE8uLDaj';           // à remplacer ou mettre en variable d'env
$CONTACT_TO        = getenv('CONTACT_TO') ?: 'benali.ramy.2@gmail.com';      // destinataire
$TURNSTILE_SECRET  = getenv('TURNSTILE_SECRET') ?: '0x4AAAAAABxow386ItI7s5HfFW5mYeVvWMU'; // secret Turnstile

// Adresse d’expéditeur (sans domaine vérifié, garde onboarding@resend.dev)
...
// Autoriser uniquement POST JSON (ou x-www-form-urlencoded)
...
if (!is_array($body)) {
  // fallback si le front envoie en form-encoded
  $body = $_POST;
}
...
$payload = [
  ...
  'reply_to' => $clean['email'], // pour répondre directement à l’expéditeur
  'text'     => $textBody,
  'html'     => $htmlBody,
];
